...
class ProductController extends Controller
{
    ...
    public function sync(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'products.*.calorie_info' => 'nullable|array',
            'products.*.calorie_info.*.amount' => 'nullable|integer|min:0',
            'products.*.calorie_info.*.unit' => 'nullable|string|max:50',
        ...
        foreach ($data['products'] as $p) {
            $attrs = [
                ...
                'features' => $p['features'] ?? [],
                'calorie_info' => $p['calorie_info'] ?? null,
                'net_weight' => $p['net_weight'] ?? null,
                'storage_condition' => $p['storage_condition'] ?? null,
                'serving_note' => $p['serving_note'] ?? null,
