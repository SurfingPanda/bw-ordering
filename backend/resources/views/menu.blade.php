    ...
    (function () {
        ...
        function renderGrid() {
            ...
            grid.innerHTML = rows.slice(0, gridLimit).map(p => {
                ...
                const calorieRows = calorieLines(p);
                return `
                <div data-view="${p.id}" role="button" tabindex="0" aria-label="View ${p.name}"
                    ...
                    {{-- Calorie info pops out above the card on hover/focus —
                         bounces in with the same back-out easing as
                         .animate-pop-in elsewhere in the app. A sibling of the
                         image box (not nested inside it) so its own
                         overflow-hidden can't clip the popped-out chip;
                         -translate-y-[calc(100%+gap)] pins it fully above
                         regardless of how many calorie rows it stacks. The
                         hovered card gets a z-index bump (see .product-card:hover
                         in app.css) so this doesn't get covered by the next
                         row's images. --}}
                    ${calorieRows.length ? `
                    <div class="pointer-events-none absolute left-1/2 top-0 z-20 flex w-max max-w-[85%] -translate-x-1/2 -translate-y-[calc(100%+0.5rem)] scale-75 items-center gap-2 rounded-2xl bg-gradient-to-br from-navy-800 to-navy-900 px-3.5 py-2 opacity-0 shadow-xl shadow-navy-900/40 ring-1 ring-white/10 transition duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:scale-100 group-hover:opacity-100 group-focus:scale-100 group-focus:opacity-100">
                        ...
                        <span class="flex flex-col gap-0.5 text-left text-xs font-bold leading-snug text-white">
                            ${calorieRows.map(line => `<span>${line}</span>`).join('')}
                        </span>
                    </div>` : ''}
