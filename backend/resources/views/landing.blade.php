@php
    ...
    $canonical = $siteUrl;
    // Verified social profiles for schema `sameAs` — pulled from the CMS
    // social links (footer icons) so adding a profile there feeds SEO too.
    // Falls back to the known Facebook page if none are configured.
    $sameAs = collect($content['social'] ?? [])
        ->only(['facebook', 'instagram', 'tiktok', 'x', 'twitter', 'youtube'])
        ->map(fn ($v) => trim((string) $v))
        ->filter(fn ($v) => (bool) preg_match('#^https?://#i', $v))
        ->values()->all();
    if (empty($sameAs)) {
    ...
    (function () {
        ...
        function renderCardControls() {
            document.querySelectorAll('[data-cart-control]').forEach(function (el) {
                ...
                var qty = cart[id] || 0;
                el.innerHTML = qty === 0
                    ? `<button type="button" data-add-to-cart="${id}" aria-label="Add to cart" class="inline-flex items-center gap-1.5 rounded-lg bg-navy-800 px-3 py-2 text-xs font-semibold text-white transition duration-200 hover:-translate-y-0.5 hover:bg-brand-600"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg><span class="hidden sm:inline">Add</span></button>`
                    : `<div class="flex items-center gap-1.5">
                        <span class="flex items-center gap-1 whitespace-nowrap rounded-lg bg-brand-50 px-2 py-1.5 text-xs font-semibold text-brand-600">
                        ...
                        <button type="button" data-add-to-cart="${id}" aria-label="Add another" class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-navy-100 text-base font-bold text-navy-800 transition hover:bg-brand-500 hover:text-white">+</button>
                       </div>`;
